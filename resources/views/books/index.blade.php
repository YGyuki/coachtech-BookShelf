<form method="POST" action="{{ route('logout') }}" novalidate>ログアウト</form>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <a href="{{ route('books.show', $book) }}"
                            ...
                            <div class="flex flex-wrap gap-1 mb-2">
                                ...
                                <span class="bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded">{{ $genre->name
                                    }}</span>
                                @endforeach
                            </div>
                            ...
                            <div class="flex items-center">
                                <span class="text-yellow-500">
                                    @for($i = 1; $i <= 5; $i++) @if($i <=round($book->reviews_avg_rating))
                                        ★
                                        @else
                                        ☆
                                        @endif
